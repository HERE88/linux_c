...
main(int argc, char **argv)
{
    ...
    if ( (childpid = Fork()) == 0) {
#ifdef AFUNIX_TEST
            if((listenfd = serv_listen("/tmp/test0928_server")) < 0)
            {
                printf("serv_listen failed:%d - %s", errno, strerror(errno));
                return -1;
            }

            if((datapipe[1] = accept(listenfd, NULL, NULL)) < 0)
            {
...
#else
            if((listenfd = open_listenfd("12888")) < 0)
            {
...
#ifdef AFUNIX_TEST
    usleep(1*1000*1000);
    datapipe[0] = cli_conn("/tmp/test0928_server"); /* Connect to server as client. */
#else
    usleep(1*1000*1000);
    datapipe[0] = open_clientfd("127.0.0.1", "12888"); /* Connect to server as client. */
#endif
    ...
    printf("AFU bandwidth: %.3f MB/sec\n",
