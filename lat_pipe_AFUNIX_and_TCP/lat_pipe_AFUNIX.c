...
main(int argc, char **argv)
{
	...
	if ( (childpid = Fork()) == 0) {
#ifdef AFUNIX_TEST
        if((listenfd = serv_listen("/tmp/test0929_server")) < 0)
        {
            printf("serv_listen failed:%d - %s", errno, strerror(errno));
            return -1;
        }
        
        if((pipe1[0] = accept(listenfd, NULL, NULL)) < 0)
        {
...
#else
        if((listenfd = open_listenfd("12889")) < 0)
        {
...
#ifdef AFUNIX_TEST
    usleep(1*1000*1000);
    pipe2[0] = cli_conn("/tmp/test0929_server"); /* Connect to server as client. */
#else
    usleep(1*1000*1000);
    pipe2[0] = open_clientfd("127.0.0.1", "12889"); /* Connect to server as client. */
#endif
		/* 4parent */
	doit(pipe2[0], pipe2[0]);

	Start_time();
	for (i = 0; i < nloop; i++)
		doit(pipe2[0], pipe2[0]);
	printf("AFUNIX latency: %.3f usec\n", Stop_time() / nloop);
