...
int kill_procs(char * proc_name)
{
    int pid_t[32]; //default max num of searches is: 32
    int ret, i;
    int ret_temp, index_i;
    memset(pid_t, 0, sizeof(pid_t));

    ret = isProcessRunning(proc_name, pid_t, sizeof(pid_t)/sizeof(int));
    if(ret != 0)
    {
        printf("No %s running.\n", proc_name);
        return 0;
    ...
    printf("%s is running, will kill it!\n", proc_name);
    for(index_i = 0; pid_t[index_i] != 0; index_i++)
    {
        ret_temp = kill(pid_t[index_i], SIGTERM);
        printf("kill %s, [pid=%d] ret_temp=%d.\n", proc_name, pid_t[index_i], ret_temp);
    }
    return 0;
}
...
int main()
{
    ...
    if(ret == -1)
    {
        printf("my_lock error, will kill test_elog!\n");
        perror("error");
        return -1;
    }
    printf("my_lock success2\n");


    /* Do whatever need here: */
    //.....

    printf("Begin sleep 15s...\n");
    sleep(15);
    printf("End sleep 15s!\n");

    ret = my_unlock(fd);
    if(ret != 0)
    {
        printf("my_unlock error!\n");
    }
    else
    {
        printf("my_unlock success.\n");
    }
