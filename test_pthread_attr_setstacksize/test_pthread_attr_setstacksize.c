...
int main(int argc, char *argv[])
{
    int ret_val = 0;
    size_t stacksize;
    size_t stacksize2 = 20*1024*1024;  // 20MB
    ...
    if (0 != ret_val)
    {
 	...
 	ret_val = pthread_attr_getstacksize(&thread_attr, &stacksize);
 	if (0 != ret_val)
    {
            LOG_PRINT("pthread_attr_getstacksize failed");
            return -1;
    }
    LOG_PRINT("default stacksize: %lu bytes\n", stacksize);

 	ret_val = pthread_attr_setstacksize(&thread_attr, stacksize2);
 	if (0 != ret_val)
    {
            LOG_PRINT("pthread_attr_setstacksize failed");
            return -1;
    }
 	ret_val = pthread_attr_getstacksize(&thread_attr, &stacksize);
 	if (0 != ret_val)
    ...
    LOG_PRINT("after stacksize: %lu bytes\n", stacksize);
